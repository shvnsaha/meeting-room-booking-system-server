...
const createRoomValidationSchema = z.object({
    body: z.object({
        ...
        }).int().nonnegative("Room number must be a non-negative integer"),

        floorNo: z.number({
            required_error: "Floor number is required",
            invalid_type_error: "Floor number must be a number"
        }).int().nonnegative("Floor number must be a non-negative integer"),

        capacity: z.number({
            required_error: "Capacity is required",
            invalid_type_error: "Capacity must be a number"
        }).int().nonnegative("Capacity must be a non-negative integer"),
